...
Эти фото послужили эталонным распределением визуальных признаков для heavy-DR сцены: в Unity я воспроизвёл дубовый паркет с per-plank tint и Perlin-grain'ом, гладкую белую поверхность (имитация той самой столешницы как левой стены) как один из стилей стены и неравномерное освещение через per-wall albedo-jitter.
...
| **rev24** (heavy-DR) | 200k шагов transfer от rev16 → пол с досками + смешанные стили стен + варьирование яркости | 35% (но prior на реале сменился на DirForward 36–59%) |
...
| **rev39** (Plan 4) | rev37 baseline + варьирование стартовой позы, динамики, асимметрии моторов, наклона камеры и шума сонара | 47% progress + полный набор sim2real-факторов вариативности |
