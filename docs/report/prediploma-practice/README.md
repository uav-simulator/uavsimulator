...
- Продукт состоит из четырех частей: `web-ui` оператора, `CLI rusim`, Unity-симулятора с API и plugin architecture, а также Python-кода обучения модели управления.
...
## Сборка DOCX
- Использовать проектный `.venv` с `python-docx` и `Pillow`.
- Команды:
