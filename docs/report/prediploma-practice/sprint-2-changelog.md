...
Ранее в симуляторе использовался префаб Prometeo (полноразмерный автомобиль, коллайдер 34×52см) и кинематическое движение через `MovePosition()`. Оба решения не подходят для sim-to-real: префаб в 2 раза больше реального робота, кинематика игнорирует физические коллайдеры и робот мог проезжать сквозь стены.

В рамках спринта проведена переработка:

- **Правильные размеры.** Шасси 14×5×22см, с колёсами и мачтой камеры 15×7×25см. Коллайдер 15×12×25см. Визуал собран из примитивов Unity: корпус, PCB-плата, батарея, передний бампер, два ультразвуковых датчика на носу, 4 колеса-цилиндра, мачта с камерой-кубиком.
- **Velocity-based движение.** Вместо `MovePosition()` используется `Rigidbody.linearVelocity`. Робот физически не может пройти сквозь стену, корректно отталкивается при контакте.
- **Корректная позиция камеры.** Y=9см (реальная высота мачты), направлена вперёд (+Z). Это важно чтобы изображение в симуляторе и на реальном роботе имело одинаковый ракурс.
...
Первая наивная версия функции вознаграждения выявила проблему — робот учился **облизывать внутреннюю стену** для быстрого добегания до финиша (racing line, оптимальный с точки зрения времени). Визуально это выглядело неэстетично и для реального робота не годилось — в реальности подобный проход означал бы постоянный скрип по картону и поломку сенсоров.
...
В процессе реализации обнаружилось что Python и C# используют разные генераторы псевдослучайных чисел (Mersenne Twister vs Knuth subtractive). Один и тот же seed давал разные последовательности и, как следствие, разные пути в maze. Из-за этого Python и Unity строили разные геометрии, и робот оказывался в другом месте относительно того, что Python считал целью.

Решение — **Python генерирует путь, Unity строит геометрию по готовому пути**. Python кодирует путь как строку `"x0,z0;x1,z1;..."` и передаёт в `trackParams["maze.path_encoded"]`. Unity парсит строку и строит стены без обращения к своему PRNG. Оба теперь работают на одной геометрии.
...
Для запуска обученной модели раньше требовалось переключаться между вкладками Control и Models. Добавлен блок AutopilotPanel прямо на основную вкладку — выбор модели, bind, start/stop, статус работы.
