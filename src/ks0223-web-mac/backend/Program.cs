...
// ─── Scenario picker ──────────────────────────────────────────────────────────
//
// GET  /api/scenarios       — list .yaml files in <repo>/configs/scenarios/
// POST /api/scenarios/load  — body { filePath } → shells out to `rusim scenario reset <file>`
//                             which parses YAML and POSTs to Unity /reset on port 8000.
//
// Both endpoints are local-only by virtue of the CORS policy above.
...
static string? ResolveRusimBinary()
{
